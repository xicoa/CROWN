...
ROOT::RDF::RNode M_dileptonMass(
    ...
    // std::vector<std::string> column_names = {particle_pts, particle_etas,
    // particle_phis, particle_masses, muon_size};
    auto df1 = df.Define(outputname, mass_calculation,
                         {particle_pts, particle_etas, particle_phis,
                          particle_masses, particle_charges, goodmuons_index});
    return df1;
}
    ...
    auto mass_calculation = [targetMass](
        ...
        for (unsigned int i = 0; i < p4_1.size(); ++i) {
            for (unsigned int j = i + 1; j < p4_2.size(); ++j) {
                ...
                auto dileptonsystem = p4_1[i] + p4_2[j];
                delta_masses.push_back(((float)dileptonsystem.mass() - targetMass));
            }
        }
        auto sortByAbs = [](float a, float b) { return abs(a) < abs(b); };
        std::sort(delta_masses.begin(), delta_masses.end(), sortByAbs);
        if (delta_masses.size() <= 0) {
            return 999.0f;
        } else {
            return delta_masses[0];
        }
    };
    auto df1 = df.Define(outputname, mass_calculation,
                         {particle_pts, particle_etas, particle_phis,
...
///
ROOT::RDF::RNode ECalGapVeto(ROOT::RDF::RNode df,
    ...
    auto lambda = [etaBoundary, lowerThresholdBarrel, upperThresholdBarrel,
        ...
        ROOT::RVec<int> mask =
            (((abs(eta) >= lowerThresholdBarrel) &&
              (abs(eta) < upperThresholdBarrel)) ||
...
ROOT::RDF::RNode DiMuonFromHiggsFlag(ROOT::RDF::RNode df,
    ...
    auto df1 = df.Define(outputname, HiggsCand_Flag, {dimuons_index});
    return df1;
}

ROOT::RDF::RNode DiLeptonFromZVetoFlag(
    ROOT::RDF::RNode df, const std::string &outputname,
    const std::string &particle_pts, const std::string &particle_etas,
    const std::string &particle_phis, const std::string &particle_masses,
    const std::string &particle_charges, const std::string &goodleptons_index) {
    auto pair_calc_mass = [](const ROOT::RVec<float> &particle_pts,
                             const ROOT::RVec<float> &particle_etas,
                             const ROOT::RVec<float> &particle_phis,
                             const ROOT::RVec<float> &particle_masses,
                             const ROOT::RVec<int> &particle_charges,
                             const ROOT::RVec<int> &goodleptons_index) {
        std::vector<ROOT::Math::PtEtaPhiMVector> p4;
        for (unsigned int k = 0; k < (int)goodleptons_index.size(); ++k) {
            try {
                p4.push_back(ROOT::Math::PtEtaPhiMVector(
                    particle_pts.at(goodleptons_index[k]),
                    particle_etas.at(goodleptons_index[k]),
                    particle_phis.at(goodleptons_index[k]),
                    particle_masses.at(goodleptons_index[k])));
            } catch (const std::out_of_range &e) {
                p4.push_back(
                    ROOT::Math::PtEtaPhiMVector(default_float, default_float,
                                                default_float, default_float));
        ...
        for (unsigned int i = 0; i < p4_1.size(); ++i) {
            for (unsigned int j = i + 1; j < p4_2.size(); ++j) {
                ...
                if (particle_charges[goodleptons_index[i]] +
                        particle_charges[goodleptons_index[j]] !=
                    0) {
                    continue;
                }
                float dilepton_mass = (p4_1[i] + p4_2[j]).mass();
                if (dilepton_mass >= 81.0 && dilepton_mass <= 101.0)
                    return 0;
            }
        }
        return 1;
    ...
    auto df1 =
        df.Define(outputname, pair_calc_mass,
                  {particle_pts, particle_etas, particle_phis, particle_masses,
                   particle_charges, goodleptons_index});
    return df1;
}
///
///
ROOT::RDF::RNode LeptonChargeSum(ROOT::RDF::RNode df,
                                 const std::string &outputname,
...
///
///
ROOT::RDF::RNode LeptonChargeSumEleMu(ROOT::RDF::RNode df,
                                      const std::string &outputname,
                                      const std::string &muon_charge,
    ...
    auto df1 = df.Define(
        outputname, calc_charge_sum,
        {muon_charge, ele_charge, goodmuons_index, base_electrons_index});
    return df1;
}
///
/// m2m and 4m channel to veto electrons
///
ROOT::RDF::RNode Ele_Veto(ROOT::RDF::RNode df, const std::string &output_name,
...
CheckDiMuon(ROOT::RDF::RNode df, const std::string &muon_collection,
            ...
            const std::string &goodmuons_index) {
    auto lambda = [](const ROOT::RVec<int> &muon_collection, const int &nmuons,
                     const int &flag, const ROOT::RVec<int> &mask,
                     const ROOT::RVec<int> &collection, const ROOT::RVec<int> & goodmuons_index) {
        static int muon_collection_2_count = 0;
        static int goodmuon_2_count = 0;
        static int muon_mask_2_count = 0;
        static int event_count = 0;
        event_count++;
        std::string msg = "";
        // if (muon_collection.size() >= 2) {
        //     muon_collection_2_count++;
        //     if (muon_collection_2_count % 100 == 0) {
        //         msg += "muon_collection with >=2 good muons: " +
        //                std::to_string(muon_collection_2_count) + "\n";
        //     }
        // }
        // if (goodmuons_index.size() >= 2){
        //     goodmuon_2_count++;
        //     if (goodmuon_2_count % 1000 == 0) {
        //         msg += "goodmuons_index with >=2 good muons: " +
        //                std::to_string(goodmuon_2_count) + "\n";
        //     }
        // }
        // if (mask.size() >= 2){
        //     muon_mask_2_count++;
        //     if (muon_mask_2_count % 1000 == 0) {
        //         msg += "muon_mask with >=2 good muons: " +
        //                std::to_string(muon_mask_2_count) + "\n";
        //     }
        // }
        // if (muon_collection.size() != nmuons) {
        //     msg += "size of muon_collection and nmuons are not equal\n";
        //     // msg += "nmuons == 1 and flag_dimuon_from_higgs == 1!\n";
        //     msg += "good muons:\t";
        //     for (auto ele : muon_collection) {
        ...
        //         msg += " " + std::to_string(ele) + "\n";
        //     }
        //     msg += "muons:\t" + std::to_string(nmuons) + "\n";
        //     msg += "flag_dimuon_from_higgs:\t" + std::to_string(flag) + "\n";
        // }
        // bool print_all = 0;
        // if (collection.size() == 0) {
        //     msg += "size of collection == 0 !";
        //     print_all = 1;
        // } else {
        //     if (event_count % 10000 == 0 || collection[0] != -1) {
        //         print_all = 1;
        //     } 
        // }
        // if (print_all) {
        ...
        //         msg = "\n" + msg;
        //     }
        //     Logger::get("CheckDiMuon")->warn(msg);
        // }
    };
    df.Foreach(lambda, {muon_collection, nmuons, flag_dimuon, muon_mask,
                        dimuon_collection, goodmuons_index});
    return df;
}
///
///
...
//                     continue;
//                 }
//                 if ((p4_1[i] + p4_2[j]).mass() < 110 ||
//                     (p4_1[i] + p4_2[j]).mass() > 150) {
//                     continue;
//                 }
...
//         return DiMuonPair;
//     };
//     auto df1 = df.Define(outputname, pair_calc_p4byPt,
//                          {particle_pts, particle_etas, particle_phis,
//                           particle_masses, particle_charges, goodmuons_index});
//     return df1;
// }
ROOT::RDF::RNode HiggsCandDiMuonPairCollection(
    ...
    auto pair_calc_p4byPt = [](const ROOT::RVec<float> &particle_pts,
                               ...
                               const ROOT::RVec<int> &particle_charges,
                               const ROOT::RVec<int> &goodmuons_index) {
        std::vector<std::pair<int, ROOT::Math::PtEtaPhiMVector>> muons;
        try {
            for (auto goodmuon : goodmuons_index) {
                ROOT::Math::PtEtaPhiMVector muon_p4(
                    particle_pts.at(goodmuon), particle_etas.at(goodmuon),
            ...
            throw "Not all muon avaliable!";
        }
        float ptsum = -1;
        int index1 = -1, index2 = -1;
        try {
            for (auto mu1 = muons.begin(); mu1 != muons.end(); mu1++) {
                for (auto mu2 = mu1 + 1; mu2 != muons.end(); mu2++) {
                    if (mu1->second.pt() < 0.0 || mu2->second.pt() < 0.0)
                        continue;
                    if (particle_charges[mu1->first] +
                            particle_charges[mu2->first] !=
                        0) {
                        continue;
                    }
                    if ((mu1->second + mu2->second).mass() < 110 ||
                        (mu1->second + mu2->second).mass() > 150) {
                        continue;
                    }
                    if (mu1->second.pt() + mu2->second.pt() > ptsum) {
                        ptsum = mu1->second.pt() + mu2->second.pt();
                        if (mu1->second.pt() > mu2->second.pt()) {
                            index1 = mu1->first;
                            index2 = mu2->first;
                        } else {
                            index1 = mu2->first;
...
ROOT::RDF::RNode ZCandDiElectronPairCollection(
    ROOT::RDF::RNode df, const std::string &outputname,
    const std::string &particle_pts, const std::string &particle_etas,
    const std::string &particle_phis, const std::string &particle_masses,
    const std::string &particle_charges, const std::string &particle_index) {
    auto lambda = [](const ROOT::RVec<float> &particle_pts,
                     const ROOT::RVec<float> &particle_etas,
                     const ROOT::RVec<float> &particle_phis,
                     const ROOT::RVec<float> &particle_masses,
                     const ROOT::RVec<int> &particle_charges,
                     const ROOT::RVec<int> &particle_index) {
        std::vector<ROOT::Math::PtEtaPhiMVector> p4;
        for (unsigned int k = 0; k < (int)particle_index.size(); ++k) {
            try {
                p4.push_back(ROOT::Math::PtEtaPhiMVector(
                    particle_pts.at(particle_index[k]),
                    particle_etas.at(particle_index[k]),
                    particle_phis.at(particle_index[k]),
                    particle_masses.at(particle_index[k])));
            } catch (const std::out_of_range &e) {
                p4.push_back(
                    ROOT::Math::PtEtaPhiMVector(default_float, default_float,
                                                default_float, default_float));
        ...
        for (unsigned int i = 0; i < p4_1.size(); ++i) {
            for (unsigned int j = i + 1; j < p4_2.size(); ++j) {
                if (p4_1[i].pt() < 0.0 || p4_2[j].pt() < 0.0)
                    continue;
                if (particle_charges[particle_index[i]] +
                        particle_charges[particle_index[j]] !=
                    0) {
                    continue;
                }
                if ((p4_1[i] + p4_2[j]).mass() < 70 ||
                    (p4_1[i] + p4_2[j]).mass() > 110) {
                    continue;
                }
                if (p4_1[i].pt() + p4_2[j].pt() > ptsum) {
                    ptsum = p4_1[i].pt() + p4_2[j].pt();
                    if (p4_1[i].pt() > p4_1[j].pt()) {
                        index1 = particle_index[i];
                        index2 = particle_index[j];
                    } else {
                        index1 = particle_index[j];
                        index2 =
                            particle_index[i]; /// need to return the index1 and
                                               /// index2 as goodmuons pair
                                               /// collection.
                    }
                }
            }
    ...
    auto df1 = df.Define(outputname, lambda,
                         {particle_pts, particle_etas, particle_phis,
                          particle_masses, particle_charges, particle_index});
    return df1;
}
...
ROOT::RDF::RNode DiEleFromZ(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto df1 = df.Define(outputname, ZCand_Flag, {dielectrons_index});
    return df1;
}
// Calculate p4 of two max pt objects with same type.
ROOT::RDF::RNode TwoObject_p4(
    ROOT::RDF::RNode df, const std::string &outputname,
    const std::string &particle_pts, const std::string &particle_etas,
    const std::string &particle_phis, const std::string &particle_masses,
    ...
    auto lambda = [](
                      ...
                      const ROOT::RVec<float> &particle_phis,
                      const ROOT::RVec<float> &particle_masses,
                      const ROOT::RVec<int> &diparticle_index) {
        if (diparticle_index.size() < 2 || diparticle_index[0] < 0) {
            return ROOT::Math::PtEtaPhiMVector(default_float, default_float,
                                               default_float, default_float);
        }
        // change it to first and second
        auto p4_1 = ROOT::Math::PtEtaPhiMVector(
            particle_pts.at(diparticle_index[0]),
            particle_etas.at(diparticle_index[0]),
            particle_phis.at(diparticle_index[0]),
            particle_masses.at(diparticle_index[0]));
        auto p4_2 = ROOT::Math::PtEtaPhiMVector(
            particle_pts.at(diparticle_index[1]),
            particle_etas.at(diparticle_index[1]),
            particle_phis.at(diparticle_index[1]),
            particle_masses.at(diparticle_index[1]));
        auto p4_diparticle = p4_1 + p4_2;
        // p4_diparticle = ROOT::Math::PtEtaPhiMVector(
        //     (p4_1 + p4_2).pt(), (p4_1 + p4_2).eta(), (p4_1 + p4_2).phi(),
        //     (p4_1 + p4_2).mass());
        return p4_diparticle;
    };

    auto df1 = df.Define(outputname, lambda,
                         {particle_pts, particle_etas, particle_phis,
                          particle_masses, diparticle_index});
    return df1;
}
///
/// \\\NOTICE
...
/// the combination in which the mass of the Z candidate is closer to 91 GeV is
/// chosen.
///
    ...
    auto lambda = [](const ROOT::RVec<float> &particle_pts,
        ...
        for (unsigned int i = 0; i < p4.size(); ++i) {
            for (unsigned int j = i + 1; j < p4.size(); ++j) {
                ...
                if (p4[pair1_index1].pt() < 0.0 ||
                    p4[pair1_index2].pt() < 0.0 ||
                    p4[pair2_index1].pt() < 0.0 ||
                    p4[pair2_index2].pt() < 0.0) {
                    continue;
                }
                /// need SFOS
                if (particle_charges[goodmuons_index[pair1_index1]] +
                        particle_charges[goodmuons_index[pair1_index2]] !=
                    0) {
                    continue;
                }
                if (particle_charges[goodmuons_index[pair2_index1]] +
                        particle_charges[goodmuons_index[pair2_index2]] !=
                    0) {
                    continue;
                }
                /// two muon mass and the other two muon
                float dimuon_mass_1 =
                    (p4[pair1_index1] + p4[pair1_index2]).mass();
                float dimuon_mass_2 =
                    (p4[pair2_index1] + p4[pair2_index2]).mass();
                ///
                /// Another, if two pairs both in Z window, return {-1,-1,-1,-1}
                /// at once
                ...
                if (dimuon_mass_1 > 110 && dimuon_mass_1 < 150) {
                    if (dimuon_mass_2 > 81 && dimuon_mass_2 < 101) {
                        ...
                        /// this if can select the ZCand which dimuon mass
                        /// closer to 91 GeV
                        if (fabs(dimuon_mass_2 - 91) <
                            fabs(min_mass_diff - 91)) {
        ...
        /// if no two pairs both in Z window among all the loop, return the four
        /// goodmuons index
...
///
///
ROOT::RDF::RNode QuadMuonFromZZVeto(ROOT::RDF::RNode df,
                                    const std::string &outputname,
                                    const std::string &quadmuons_index) {
    auto HiggsZCand_Flag = [](const ROOT::RVec<int> &quadmuons_index) {
        ROOT::Math::PtEtaPhiMVector p4_dimuon;
        std::vector<ROOT::Math::PtEtaPhiMVector> p4;
        if (quadmuons_index.at(0) == -1 || quadmuons_index.at(1) == -1 ||
            quadmuons_index.at(2) == -1 || quadmuons_index.at(3) == -1) {
            return 0;
        } else { // else exist four muons that may from Higgs and Z
            return 1;
        }
    };
    auto df1 = df.Define(outputname, HiggsZCand_Flag, {quadmuons_index});
    return df1;
}
///
    ...
    auto dimuonZ_calc_p4 = [](const ROOT::RVec<float> &particle_pts,
        ...
        if (fourmuons_index.at(2) == -1 || fourmuons_index.at(3) == -1) {
            ...
            for (unsigned int k = 2; k < (int)fourmuons_index.size(); ++k) {
                /// push back [2] and [3]
                p4.push_back(ROOT::Math::PtEtaPhiMVector(
                    particle_pts.at(fourmuons_index[k]),
                    particle_etas.at(fourmuons_index[k]),
                    particle_phis.at(fourmuons_index[k]),
                    particle_masses.at(fourmuons_index[k])));
            }
            p4_dimuZ = ROOT::Math::PtEtaPhiMVector(
                (p4[0] + p4[1]).pt(), (p4[0] + p4[1]).eta(),
                (p4[0] + p4[1]).phi(), (p4[0] + p4[1]).mass());
            return p4_dimuZ;
        }
    };
    auto df1 = df.Define(outputname, dimuonZ_calc_p4,
                         {particle_pts, particle_etas, particle_phis,
                          particle_masses, fourmuons_index});
    return df1;
}

/// extra muon or additional lepton or add. lepton
ROOT::RDF::RNode ExtraObjectIndex(
    ...
    if (exclude_column.empty()) {
        auto lambda = [exclude_index](const ROOT::RVec<float> &particle_pts,
                                      const ROOT::RVec<float> &particle_etas,
                                      const ROOT::RVec<float> &particle_phis,
                                      const ROOT::RVec<float> &particle_masses,
                                      const ROOT::RVec<int> &object_index) {
            for (int i = 0; i < object_index.size(); i++) {
                int this_index = object_index[i];
                for (int j = 0; i < exclude_index.size(); j++) {
                    if (this_index == exclude_index[j]) {
                        this_index = -1;
                        break;
                    }
                }
                if (this_index != -1) {
                    return ROOT::RVec<int>(1, this_index);
                }
            }
        ...
        auto lambda = [exclude_index](const ROOT::RVec<float> &particle_pts,
                                      ...
                                      const ROOT::RVec<float> &particle_masses,
                                      const ROOT::RVec<int> &object_index,
                                      const ROOT::RVec<int> &exclude_column) {
            for (int i = 0; i < object_index.size(); i++) {
                int this_index = object_index[i];
                for (int j = 0; j < exclude_column.size(); j++) {
                    if (this_index == exclude_column[j]) {
                        this_index = -1;
                        break;
                    }
                }
        ...
        auto df1 = df.Define(outputname, lambda,
                             {particle_pts, particle_etas, particle_phis,
                              particle_masses, object_index, exclude_column});
        return df1;
    }
}
/// extra object p4
ROOT::RDF::RNode ObjectWithIndex_p4(ROOT::RDF::RNode df,
    ...
    auto lambda = [](const ROOT::RVec<float> &particle_pts,
        ...
        if (particle_index[0] == -1) {
            p4_lepFromW.push_back(ROOT::Math::PtEtaPhiMVector(
                default_float, default_float, default_float, default_float));
        } else {
            p4_lepFromW.push_back(ROOT::Math::PtEtaPhiMVector(
                particle_pts.at(particle_index[0]),
                particle_etas.at(particle_index[0]),
                particle_phis.at(particle_index[0]),
                particle_masses.at(particle_index[0])));
        }
...
ROOT::RDF::RNode muSSorOSwithLeptonW_p4(
    ROOT::RDF::RNode df, const std::string &outputname,
    const std::string &muon_pts, const std::string &muon_etas,
    const std::string &muon_phis, const std::string &muon_masses,
    const std::string &muon_charges, const std::string &lep_charges,
    const std::string &dimuons_index, const std::string &lep_index,
    const int SameSign) {
        ...
        if (dimuons_index.at(0) == -1 || dimuons_index.at(1) == -1 ||
            ...
            if (SameSign == 1) {
                for (unsigned int k = 0; k < (int)dimuons_index.size(); ++k) {
                    /// same sign
                    if (lep_charges[lep_index[0]] ==
                        muon_charges[dimuons_index[k]]) {
                        p4.push_back(ROOT::Math::PtEtaPhiMVector(
                            muon_pts.at(dimuons_index[k]),
    ...
    auto df1 = df.Define(outputname, calc_p4,
                         {muon_pts, muon_etas, muon_phis, muon_masses,
                          muon_charges, lep_charges, dimuons_index, lep_index});
    return df1;
}
///
///// function to calc the MHT
ROOT::RDF::RNode MHT_Calculation(ROOT::RDF::RNode df,
                                 const std::string &outputname,
                                 const std::string &particle_pts,
                                 const std::string &particle_etas,
                                 const std::string &particle_phis,
    ...
    auto MHT_calc_p4 = [](const ROOT::RVec<float> &particle_pts,
        ...
        for (unsigned int j = 0; j < (int)goodjets_index.size(); ++j) {
            if (particle_pts.at(goodjets_index[j]) > 30 &&
                fabs(particle_etas.at(goodjets_index[j])) < 4.7) {
                p4_MHT += ROOT::Math::PtEtaPhiMVector(
                    particle_pts.at(goodjets_index[j]),
        ...
        ///
        p4_MHT = -p4_MHT;
        /// p4_MHT = -ROOT::Math::PtEtaPhiEVector(p4_MHT.Pt(), 0, p4_MHT.Phi(),
        /// p4_MHT.Pt());
        return (ROOT::Math::PtEtaPhiMVector)p4_MHT;
    };
    auto df1 = df.Define(outputname, MHT_calc_p4,
                         {particle_pts, particle_etas, particle_phis,
                          particle_masses, goodjets_index});
    return df1;
}
/// UF calculation about MHT?
ROOT::RDF::RNode
MHT_CalculationALL(ROOT::RDF::RNode df, const std::string &outputname,
                   const std::string &muon_pts, const std::string &muon_etas,
                   const std::string &muon_phis, const std::string &muon_masses,
                   const std::string &muon_index, const std::string &ele_pts,
                   const std::string &ele_etas, const std::string &ele_phis,
                   const std::string &ele_masses, const std::string &ele_index,
                   const std::string &jet_pts, const std::string &jet_etas,
                   const std::string &jet_phis, const std::string &jet_masses,
                   const std::string &goodjets_index) {
    auto MHT_calc_p4 =
        [](const ROOT::RVec<float> &muon_pts,
           ...
           const ROOT::RVec<float> &muon_phis,
           const ROOT::RVec<float> &muon_masses,
           const ROOT::RVec<int> &muon_index, const ROOT::RVec<float> &ele_pts,
           const ROOT::RVec<float> &ele_etas, const ROOT::RVec<float> &ele_phis,
           const ROOT::RVec<float> &ele_masses,
           const ROOT::RVec<int> &ele_index, const ROOT::RVec<float> &jet_pts,
           ...
           const ROOT::RVec<int> &goodjets_index) {
            ...
            for (unsigned int j = 0; j < (int)muon_index.size(); ++j) {
                p4_MHTALL += ROOT::Math::PtEtaPhiMVector(
                    muon_pts.at(muon_index[j]), muon_etas.at(muon_index[j]),
                    muon_phis.at(muon_index[j]), muon_masses.at(muon_index[j]));
            }
            /// ele
            for (unsigned int j = 0; j < (int)ele_index.size(); ++j) {
                p4_MHTALL += ROOT::Math::PtEtaPhiMVector(
                    ele_pts.at(ele_index[j]), ele_etas.at(ele_index[j]),
                    ele_phis.at(ele_index[j]), ele_masses.at(ele_index[j]));
            }
            /// jet
            for (unsigned int j = 0; j < (int)goodjets_index.size(); ++j) {
                if (jet_pts.at(goodjets_index[j]) > 30 &&
                    fabs(jet_etas.at(goodjets_index[j])) < 4.7) {
                    p4_MHTALL += ROOT::Math::PtEtaPhiMVector(
                        jet_pts.at(goodjets_index[j]),
            ...
            ///
            p4_MHTALL = -p4_MHTALL;
            /// p4_MHT = -ROOT::Math::PtEtaPhiEVector(p4_MHT.Pt(), 0,
            /// p4_MHT.Phi(), p4_MHT.Pt());
            return (ROOT::Math::PtEtaPhiMVector)p4_MHTALL;
        };
    auto df1 =
        df.Define(outputname, MHT_calc_p4,
                  {muon_pts, muon_etas, muon_phis, muon_masses, muon_index,
                   ele_pts, ele_etas, ele_phis, ele_masses, ele_index, jet_pts,
                   jet_etas, jet_phis, jet_masses, goodjets_index});
    return df1;
}
///
...
ROOT::RDF::RNode PassFlag(ROOT::RDF::RNode df, const std::string &outputname) {
    auto Pass_Flag = []() { return 1; };
    auto df1 = df.Define(outputname, Pass_Flag, {});
    return df1;
}
///
ROOT::RDF::RNode PassDiEleIn4m(ROOT::RDF::RNode df,
                               const std::string &outputname) {
    auto Pass = []() { return 999.0f; };
    auto df1 = df.Define(outputname, Pass, {});
    return df1;
}
///
ROOT::RDF::RNode Calc_CosThetaStar(ROOT::RDF::RNode df,
                                   const std::string &outputname,
                                   const std::string &lepton_p4,
                                   const std::string &mu_p4) {
    auto calculate_costhstar = [](ROOT::Math::PtEtaPhiMVector &lep_p4,
        ...
        lepton_p4_TL.SetPtEtaPhiM(lep_p4.Pt(), lep_p4.Eta(), lep_p4.Phi(),
                                  lep_p4.M());
...
///
///
ROOT::RDF::RNode Calc_CosThetaStar_ZH(ROOT::RDF::RNode df,
                                      const std::string &outputname,
                                      const std::string &Z_p4,
                                      const std::string &H_p4) {
...
///
ROOT::RDF::RNode RedirectZlepID(ROOT::RDF::RNode df, const int ifMu,
                                const std::string &outputname) {
    auto LepID = [ifMu]() {
        if (ifMu == 1) {
            return 13;
    ...
    auto df1 = df.Define(outputname, LepID, {});
    return df1;
}
///
//// met_p4 PtEtaPhiM
...
ROOT::RDF::RNode MetCut(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto df1 = df.Define(outputname, cut_met, {met_p4});
    return df1;
}
...
ROOT::RDF::RNode
HtScalar(ROOT::RDF::RNode df, const std::string &outputname,
         const std::string &muons_pt, const std::string &good_muons_collection,
    ...
    auto calculate_htscalar =
        [max_jet_eta](const ROOT::RVec<float> &muons_pt,
                       ...
                       const ROOT::RVec<float> &jets_pt,
                       const ROOT::RVec<float> &jets_eta,
                       const ROOT::RVec<int> &good_jets_collection) {
        float ht_scalar = 0.0;
        for (unsigned int i = 0; i < good_muons_collection.size(); ++i) {
            int index = good_muons_collection[i];
            if (index >= 0 && muons_pt[index] >= 0) {
                ht_scalar += muons_pt[index];
            }
        }
        for (unsigned int i = 0; i < good_electrons_collection.size(); ++i) {
            int index = good_electrons_collection[i];
            if (index >= 0 && electrons_pt[index] >= 0) {
                ht_scalar += electrons_pt[index];
            }
        }
        for (unsigned int i = 0; i < good_jets_collection.size(); ++i) {
            int index = good_jets_collection[i];
            if (index >= 0 && jets_pt[index] >= 0 && fabs(jets_eta[index]) < max_jet_eta) {
                ht_scalar += jets_pt[index];
            }
        }
        return ht_scalar;
    };
    auto df1 = df.Define(outputname, calculate_htscalar,
                         {muons_pt, good_muons_collection, electrons_pt,
                          good_electrons_collection, jets_pt, jets_eta,
                          good_jets_collection});
    return df1;
}
...
ROOT::RDF::RNode HtVectorPtAndPhi(
    ...
    const std::string &good_muons_collection, const std::string &electrons_pt,
    ...
    auto calculate_htvector =
        [max_jet_eta](const ROOT::RVec<float> &muons_pt,
                       ...
                       const ROOT::RVec<int> &good_muons_collection,
                       const ROOT::RVec<float> &electrons_pt,
                       const ROOT::RVec<float> &electrons_phi,
                       const ROOT::RVec<int> &good_electrons_collection,
                       const ROOT::RVec<float> &jets_pt,
        ...
        for (unsigned int i = 0; i < good_muons_collection.size(); ++i) {
            int index = good_muons_collection[i];
            if (index >= 0 && muons_pt[index] >= 0) {
                ht_x += muons_pt[index] * cos(muons_phi[index]);
                ht_y += muons_pt[index] * sin(muons_phi[index]);
            }
        }
        for (unsigned int i = 0; i < good_electrons_collection.size(); ++i) {
            int index = good_electrons_collection[i];
            if (index >= 0 && electrons_pt[index] >= 0) {
                ht_x += electrons_pt[index] * cos(electrons_phi[index]);
                ht_y += electrons_pt[index] * sin(electrons_phi[index]);
            }
        }
        for (unsigned int i = 0; i < good_jets_collection.size(); ++i) {
            int index = good_jets_collection[i];
            if (index >= 0 && jets_pt[index] >= 0 && fabs(jets_eta[index]) < max_jet_eta) {
                ht_x += jets_pt[index] * cos(jets_phi[index]);
                ht_y += jets_pt[index] * sin(jets_phi[index]);
            }
        }
        float ht_pt = sqrt(ht_x * ht_x + ht_y * ht_y);
        float ht_phi = atan2(ht_y, ht_x);
        Logger::get("HtVectorPtAndPhi")
            ->debug("ht_pt = {}, ht_phi = {}, ht_x = {}, ht_y = {}", ht_pt,
                    ht_phi, ht_x, ht_y);
        ROOT::RVec<float> ht_vector = {ht_pt, ht_phi};
        return ht_vector;
    };
    auto df1 =
        df.Define(outputname, calculate_htvector,
                  {muons_pt, muons_phi, good_muons_collection, electrons_pt,
                   electrons_phi, good_electrons_collection, jets_pt, jets_phi,
                   jets_eta, good_jets_collection});
    return df1;
}
...
ROOT::RDF::RNode HiggsCandDiMuonGenPairCollection(
    ...
    auto RecoToGen = [](const ROOT::RVec<int> &Muon_indexToGen,
        ...
        int index1, index2;
        index1 = Muon_indexToGen.at(dimuon_index[0], -1);
        index2 = Muon_indexToGen.at(dimuon_index[1], -1);
...
ROOT::RDF::RNode BosonDecayMode(ROOT::RDF::RNode df,
    ...
    auto DecayMode = [](const ROOT::RVec<int> &GenPart_pdgId,
        ...
        for (unsigned int i = 0; i < (int)GenPart_pdgId.size(); ++i) {
            ...
            if ((abs(GenPart_pdgId.at(i)) == 24) &&
                ((GenPart_statusFlags.at(i) >> 13) & 1 == 1)) {
                ...
                for (unsigned int j = 0; j < (int)GenPart_pdgId.size(); ++j) {
                    ...
                    if (GenPart_motherid.at(j) == i) {
                        ...
                        if (std::find(ids_lepton.begin(), ids_lepton.end(),
                                      abs(GenPart_pdgId.at(j))) !=
                            ids_lepton.end()) {
                            return 0; // 0 stands for W leptonic decay
                        } else if (std::find(ids_hadron.begin(),
                                             ids_hadron.end(),
                                             abs(GenPart_pdgId.at(j))) !=
                                   ids_hadron.end()) {
            ...
            } else if ((abs(GenPart_pdgId.at(i)) == 23) &&
                       ((GenPart_statusFlags.at(i) >> 13) &
                        1 == 1)) { // gen particle is Z
                // check gen particle's daughter
                for (unsigned int k = 0; k < (int)GenPart_pdgId.size(); ++k) {
                    // find the gen part daughter
                    if (GenPart_motherid.at(k) == i) {
                        // return abs(GenPart_pdgId.at(k));
                        if (std::find(ids_lepton_Z.begin(), ids_lepton_Z.end(),
                                      abs(GenPart_pdgId.at(k))) !=
                            ids_lepton_Z.end()) {
                            return 0; // 0 stands for Z leptonic decay
                        } else if (std::find(ids_hadron_Z.begin(),
                                             ids_hadron_Z.end(),
                                             abs(GenPart_pdgId.at(k))) !=
                                   ids_hadron_Z.end()) {
                            return 1; // 1 stands for Z hadronic decay
                        } else if (std::find(ids_invis.begin(), ids_invis.end(),
                                             abs(GenPart_pdgId.at(k))) !=
                                   ids_invis.end()) {
    ...
    auto df1 =
        df.Define(outputname, DecayMode,
                  {GenPart_pdgId, GenPart_motherid, GenPart_statusFlags});
    return df1;
}
/// function to pick dimuon pair in DY control region
ROOT::RDF::RNode DY_DiMuonPair_CR(
    ROOT::RDF::RNode df, const std::string &outputname,
    const std::string &particle_pts, const std::string &particle_etas,
    const std::string &particle_phis, const std::string &particle_masses,
    const std::string &particle_charges, const std::string &goodmuons_index) {
            ...
            for (unsigned int j = i + 1; j < p4_2.size(); ++j) {
                ...
                if (p4_1[i].pt() + p4_2[j].pt() > ptsum) {
                    ...
                    if (p4_1[i].pt() > p4_1[j].pt()) {
                        ...
                        index1 = goodmuons_index[j];
                        index2 =
                            goodmuons_index[i]; /// need to return the index1
                                                /// and index2 as goodmuons pair
                                                /// collection.
                    }
                }
            }
...
ROOT::RDF::RNode DiMuonFromCR(ROOT::RDF::RNode df,
                              const std::string &outputname,
                              const std::string &dimuons_index) {
    auto lambda = [](const ROOT::RVec<int> &dimuons_index) {
        ROOT::Math::PtEtaPhiMVector p4_dimuon;
        std::vector<ROOT::Math::PtEtaPhiMVector> p4;
        if (dimuons_index.at(0) == -1 || dimuons_index.at(1) == -1) {
            return 0;
    ...
    auto df1 = df.Define(outputname, lambda, {dimuons_index});
    return df1;
}
///
/// function to use dimuon pair index to get dmuon p4 in control region
ROOT::RDF::RNode ZControlDiMuonPairP4(ROOT::RDF::RNode df,
                                      const std::string &outputname,
                                      const std::string &particle_pts,
                                      const std::string &particle_etas,
                                      const std::string &particle_phis,
    ...
    auto dimuon_calc_p4byPt = [](const ROOT::RVec<float> &particle_pts,
        ...
        if (dimuons_index.at(0) == -1 || dimuons_index.at(1) == -1) {
            ...
            for (unsigned int k = 0; k < (int)dimuons_index.size(); ++k) {
                p4.push_back(ROOT::Math::PtEtaPhiMVector(
                    particle_pts.at(dimuons_index[k]),
                    particle_etas.at(dimuons_index[k]),
                    particle_phis.at(dimuons_index[k]),
                    particle_masses.at(dimuons_index[k])));
            }
            p4_dimuon = ROOT::Math::PtEtaPhiMVector(
                (p4[0] + p4[1]).pt(), (p4[0] + p4[1]).eta(),
                (p4[0] + p4[1]).phi(), (p4[0] + p4[1]).mass());
            return p4_dimuon;
        }
    };
    auto df1 = df.Define(outputname, dimuon_calc_p4byPt,
                         {particle_pts, particle_etas, particle_phis,
                          particle_masses, dimuons_index});
    return df1;
}
///
/// function to pick ele muon pair in Top control region
ROOT::RDF::RNode
TOP_EleMuPair_CR(ROOT::RDF::RNode df, const std::string &outputname,
                 const std::string &muon_pts, const std::string &muon_etas,
                 const std::string &muon_phis, const std::string &muon_masses,
                 const std::string &muon_charges,
                 const std::string &goodmuons_index, const std::string &ele_pts,
                 const std::string &ele_etas, const std::string &ele_phis,
                 const std::string &ele_masses, const std::string &ele_charges,
    ...
    auto pair_calc_p4byPt = [](const ROOT::RVec<float> &muon_pts,
        ...
        for (unsigned int k = 0; k < (int)goodmuons_index.size(); ++k) {
            try {
                p4_mu.push_back(ROOT::Math::PtEtaPhiMVector(
                    muon_pts.at(
                        goodmuons_index[k]), /// goodmuons_index[k] points to
                                             /// the good muon index k
                    muon_etas.at(goodmuons_index[k]), // k = 0, points to
                                                      // goodmuon_index[0]
                    muon_phis.at(
                        goodmuons_index[k]), // k ,points to goodmuon_index[k]
                    muon_masses.at(
        ...
        for (unsigned int k = 0; k < (int)baseeles_index.size(); ++k) {
            try {
                p4_ele.push_back(ROOT::Math::PtEtaPhiMVector(
                    ele_pts.at(baseeles_index[k]), /// goodmuons_index[k] points
                                                   /// to the good muon index k
                    ele_etas.at(baseeles_index[k]), // k = 0, points to
                                                    // goodmuon_index[0]
                    ele_phis.at(
                        baseeles_index[k]), // k ,points to goodmuon_index[k]
                    ele_masses.at(
                        baseeles_index[k]))); // index what I want is
                                              // goodmuon_index[k] k,i or j
            } catch (const std::out_of_range &e) {
                p4_ele.push_back(
                    ROOT::Math::PtEtaPhiMVector(default_float, default_float,
        ...
        for (unsigned int i = 0; i < p4_mu.size(); ++i) {
            for (unsigned int j = 0; j < p4_ele.size(); ++j) {
                ...
                if (muon_charges[goodmuons_index[i]] +
                        ele_charges[baseeles_index[j]] !=
                    0) {
                    continue;
                }
                /// Add dimuon mass window, 70,110,150
                if ((p4_mu[i] + p4_ele[j]).mass() < 110 ||
                    (p4_mu[i] + p4_ele[j]).mass() > 150) {
                    continue;
                }
    ...
    auto df1 = df.Define(outputname, pair_calc_p4byPt,
                         {muon_pts, muon_etas, muon_phis, muon_masses,
                          muon_charges, goodmuons_index, ele_pts, ele_etas,
                          ele_phis, ele_masses, ele_charges, baseeles_index});
    return df1;
}
///
...
ROOT::RDF::RNode EleMuFromCR(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto df1 = df.Define(outputname, EleMuCand_Flag, {elemu_index});
    return df1;
}
///
/// function to use ele mu pair index to get ele mu p4 in top control region
/// elemu_index, index[0] stands mu, index[1] stands ele
ROOT::RDF::RNode TopControlEleMuPairP4(
    ROOT::RDF::RNode df, const std::string &outputname,
    const std::string &muon_pts, const std::string &muon_etas,
    const std::string &muon_phis, const std::string &muon_masses,
    const std::string &ele_pts, const std::string &ele_etas,
    const std::string &ele_phis, const std::string &ele_masses,
    const std::string &elemu_index) {
    auto elemu_calc_p4byPt = [](const ROOT::RVec<float> &muon_pts,
        ...
        if (elemu_index.at(0) == -1 || elemu_index.at(1) == -1) {
            ...
            p4.push_back(ROOT::Math::PtEtaPhiMVector(
                muon_pts.at(elemu_index[0]), muon_etas.at(elemu_index[0]),
                muon_phis.at(elemu_index[0]), muon_masses.at(elemu_index[0])));
            p4.push_back(ROOT::Math::PtEtaPhiMVector(
                ele_pts.at(elemu_index[1]), ele_etas.at(elemu_index[1]),
                ele_phis.at(elemu_index[1]), ele_masses.at(elemu_index[1])));
            p4_elemu = ROOT::Math::PtEtaPhiMVector(
                (p4[0] + p4[1]).pt(), (p4[0] + p4[1]).eta(),
                (p4[0] + p4[1]).phi(), (p4[0] + p4[1]).mass());
            return p4_elemu;
        }
    };
    auto df1 = df.Define(outputname, elemu_calc_p4byPt,
                         {muon_pts, muon_etas, muon_phis, muon_masses, ele_pts,
                          ele_etas, ele_phis, ele_masses, elemu_index});
    return df1;
}
///
/// function  to make a flag that if exist dimuon pair from Higgs
ROOT::RDF::RNode
LeadingFatJetSoftDropMass(ROOT::RDF::RNode df, const std::string &outputname,
    ...
    auto df1 = df.Define(outputname, FatJetSDM,
                         {fatjet_softdrop_mass, good_fatjets_index});
    return df1;
}
///
ROOT::RDF::RNode LeadingFatJetVar(ROOT::RDF::RNode df,
                                  const std::string &outputname,
...
// ROOT::RDF::RNode HiggsToMuMu_Cand(ROOT::RDF::RNode df, const std::string
// &maskname,
...
//     const float dimuon_pt = dimuon_p4_vec.Pt(); // maybe 0123 stands for pt
//     eta phi mass? auto lambda = [dimuon_pt]() {
...
//         return output_p4;
//         };
//     return df.Define(outputname, FirstEle, {input_p4_vec[0].pt(),
//     input_p4_vec[0].eta(), input_p4_vec[0].phi(), input_p4_vec[0].mass()});
// }
/// end write by botao
...
ROOT::RDF::RNode CutVarMaxUChar(ROOT::RDF::RNode df,
    ...
    auto lambda = [threshold](const ROOT::RVec<unsigned char> &values) {
...
/// regionThreshold
/// \param[in] thresholdAbove upper threshold applied when regionvar greater
/// than regionThreshold \param[in] regionThreshold seperate regionvar into
/// below and above region \param[in] absMode compare abs(regionvar) to
/// regionThreshold instead of regionvar \return a dataframe containing the new
/// mask
ROOT::RDF::RNode
CutVarMaxPiecewise(ROOT::RDF::RNode df, const std::string &quantity,
    ...
    auto lambda = [&thresholdBelow, &thresholdAbove, &regionThreshold,
                   &absMode](const ROOT::RVec<float> &q_i,
                             const ROOT::RVec<float> &r_i) {
...
/// function is a combination of CutVarMaxClosestObj and CutVarMaxPiecewise.
///
/// add by hao
///
/// \param[in] df the input dataframe
